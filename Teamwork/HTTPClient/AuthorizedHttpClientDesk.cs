...
namespace Teamwork.HTTPClient
{
    ...
    /// </summary>
   public partial class AuthorizedHttpClientDesk : HttpClient
   {
      /// <summary>
      ...
      public AuthorizedHttpClientDesk(string pApiKey, Uri pBaseuri)
        {
        ...
        /// <param name="pApiKey">APIKey for Projects API</param>
        /// <param name="pBaseuri"></param>
        public AuthorizedHttpClientDesk(string pApiKey, Uri pBaseuri, HttpMessageHandler handler) : base(handler)
        {
            BaseAddress = pBaseuri;
            DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.UTF8.GetBytes($"{pApiKey}:x")));
            DefaultRequestHeaders.Accept.Clear();
        ...
        public AuthorizedHttpClientDesk()
        {
            DefaultRequestHeaders.Accept.Clear();
            DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        }
